							...
							char * response, char * save_to_file, int * ret_code)
{
	...
	int index = 0, index_packet = 0, recvd = 0, size_to_receive = 0, total_size = 0, i = 0, size_block = 0, len_recv = 0, ret_size = 0, header_size = 0;
	unsigned char isMultiplePackets = 0;
	FILE *fp = NULL;
	...
	ei_encode_list_header(buf, &index, 7);
	...
	// elemento binário número do terminal
	ei_encode_binary(buf, &index, logical_number, strlen(logical_number));
	// elemento binário nome do aplicativo que está sendo executado
	ei_encode_binary(buf, &index, app_name, strlen(app_name));
	...
	// receiving header sz
	recvd = UCLReceive(packet_recv, 4);
	if (recvd <= 0) 
	{
		return -1;
	}
	...
	// header size
	header_size = MAKELONG(MAKEWORD(packet_recv[3],packet_recv[2]),MAKEWORD(packet_recv[1],packet_recv[0]));
	// printf("recvd = %d header_size = %d\n", recvd, header_size);
	if (header_size > 1024) 
	{
		header_size = 1024;
	}
	...
	// receiving buffer ...
	memset(packet_recv,0,sizeof(packet_recv));
	recvd = UCLReceive(packet_recv, header_size);
	...
	for(i = 0; i < recvd; i++)
	{
		if (packet_recv[i] == 0x83 && packet_recv[i+1] == 0x6C && packet_recv[i+2] == 0x00) break;
	}
	...
	*ret_code = packet_recv[i+7];
	...
	if (save_to_file)
	{
			...
			if (ret_size > 0)
			{
				...
				if (fp == NULL) return -3;
				if (size_to_receive <= 0)	fwrite(&packet_recv[i+13],1,ret_size,fp);
	...
	while(size_to_receive > 0)
	{
		...
		if (save_to_file && fp)
		{
			if ((size_to_receive - len_recv) <= 0)
				fwrite(packet_recv,1,size_to_receive,fp);
			else
	...
	//clean last bytes from mapreduce request
	if (isMultiplePackets)
	{
		//receive 6A
		recvd = UCLReceive(packet_recv, 1);
		printf("(1) isMultiplePackets = true recvd = %d\n", recvd);
		//receive footer from mapreduce
		recvd = UCLReceive(packet_recv, 7);
		printf("(2) isMultiplePackets = true recvd = %d\n", recvd);
	}
	else
	{
		//receive footer from mapreduce
		recvd = UCLReceive(packet_recv, 7);
		printf("isMultiplePackets = false recvd = %d\n", recvd);
	}
...
int main(int argc, char *argv[])
{
	...
	ret = riak_mapreduce_request(	"assets", "pp1_eeee", "walk", "get_asset", 											// bucket, key, module, function
									"280-031-301", "3.01", "inicio.posxml", "FFFF", "0,AAAAA,err,sn", "0", "inicio.posxml",	// serial, version, app, crc, buffer, logical_number, app_name
									buf, "pp1_eeee_2.txt", &ret_code);							
	printf("ret = %d ret_code = %d\n", ret, ret_code);
		
	// calling mapreduce_request to get a file, posxml: baixaarquivo
	memset(buf,0,sizeof(buf));
	ret = riak_mapreduce_request(	"assets", "pp1_ffff", "walk", "get_asset", 											// bucket, key, module, function
									"280-031-301", "3.01", "inicio.posxml", "FFFF", "0,AAAAA,err,sn", "0", "inicio.posxml",	// serial, version, app, crc, buffer, logical_number, app_name
									buf, "pp1_ffff.txt", &ret_code);							
	printf("ret = %d ret_code = %d\n", ret, ret_code);
	...
	// 									"000-000-000", "3.01", "inicio.posxml", "FFFF", "0,AAAAA,err,sn", "0", "inicio.posxml",	// serial, version, app, crc, buffer, logical_number, app_name
	// 									buf, NULL, &ret_code);
	// printf("ret = %d ret_code = %d\n", ret, ret_code);
	// hex_dump(buf, ret, "response");
